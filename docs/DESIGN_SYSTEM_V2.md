...
`PublicProfileComponents` and route-split `profiles.css` extend the established public system for Supplier/Venue portfolios. Reuse `PublicProfileHero` over PublicPageHeader, `MediaGallery`, `ProfileLayout`, `ProfileSection`, `FeatureList`, `ReviewSummary/ReviewList`, `LocationSection` and `StickyEnquiryCard`. `/design-system` includes a clearly synthetic profile example. Use a 48px desktop profile title (40px tablet/32px mobile), 28px section headings, photography with reserved geometry, plain divided sections and one pastel enquiry panel. The photo viewer uses the existing Radix Dialog primitive and Lucide controls. Do not create packages, reviews, maps, availability or contact capabilities without an existing data/workflow contract.

### Category and SEO landing pattern

Use `PublicSeoComponents` for organic landing pages: SeoLandingHeader over PublicPageHeader, SeoCategoryCard (retaining full category copy), SeoSupplierResults using SupplierCard's list layout, SeoResults for distinct states, and SeoPagination. Keep the existing H1/intro, canonical, schema, slug and internal-link contracts. The synthetic `/design-system` example demonstrates category cards and pagination. Shared tokens now live in `public-tokens.css`, imported unchanged by `public.css` and reused by the static SEO presentation adapter. `seo-prerender.css` restyles the existing crawlable list markup without replacing its content. Do not fabricate FAQ, nearby-location, subcategory or editorial content to fill a layout.

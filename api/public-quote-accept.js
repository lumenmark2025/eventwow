  ...
  try {
    ...
    if (status !== "sent") {
      return res.status(409).json({
        ok: false,
        error: "Cannot accept quote",
        details: "Only sent quotes can be accepted",
      });
    }
    ...
    if (updateErr) {
      return res.status(500).json({ ok: false, error: "Failed to accept quote", details: updateErr.message });
    }
    ...
    if (!updated) {
      ...
      if (latestErr || !latest) {
        return res.status(409).json({ ok: false, error: "Cannot accept quote", details: "Quote changed" });
      }
    ...
    console.error("public-quote-accept crashed:", err);

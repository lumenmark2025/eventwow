...
function budgetRangeFromAmount(amount, unit) {
  ...
  const formatted = Number.isInteger(safeAmount)
    ? safeAmount.toFixed(0)
    : safeAmount.toFixed(2);
  return unit === "per_person"
    ? `£${formatted} per person`
    : `£${formatted} in total`;
}
...
function buildSuggestionHints(form) {
  ...
  if (!form.venue_name && !form.venue_postcode)
    hints.push("add venue name or postcode");
  if (!form.start_time) hints.push("add preferred serving time");
  if (!form.dietary_requirements)
    hints.push("add dietary requirements if relevant");
  return hints;
}
...
export default function SupplierRequestQuotePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!resp.ok)
          throw new Error(
            json?.details || json?.error || "Supplier unavailable",
          );
        if (!mounted) return;
        setContext(json);
        ...
        setForm((prev) => ({
          ...prev,
          enquiry_category_slug:
            json?.activeCategoryCount === 1
              ? firstCategorySlug
              : prev.enquiry_category_slug,
        }));
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        const resp = await fetch(
          `/api/public-venues-search?q=${encodeURIComponent(q)}`,
        );
        const json = await resp.json().catch(() => ({}));
        if (!resp.ok)
          throw new Error(
            json?.details || json?.error || "Venue search failed",
          );
        if (!mounted) return;
  ...
  async function onSubmit(e) {
    ...
    if (
      ...
      !parseGuestCount(form.guest_count)
    ) {
    ...
    try {
      ...
      const effectiveBudgetUnit = parsedBudgetAmount
        ? form.budget_unit || "in_total"
        : null;
      const payload = {
        ...
        budget_range:
          form.budget_range ||
          budgetRangeFromAmount(parsedBudgetAmount, effectiveBudgetUnit),
      ...
      if (!resp.ok) {
        ...
        throw new Error(
          json?.details || json?.error || "Failed to submit request",
        );
      }
    ...
    return (
      <MarketingShell>
        <EmptyState
          ...
          description={error || "Please try another supplier."}
        />
      </MarketingShell>
    );
  }
  ...
  return (
    <MarketingShell>
      <section className="journey-request space-y-5">
        <PublicPageHeader
          title={`Request a quote from ${context.supplierName}`}
          subtitle="Share structured event details so suppliers can respond with better quotes."
          breadcrumb="Request a quote"
        />
        <Card>
          <CardContent>
            <form onSubmit={onSubmit} className="space-y-5">
              <JourneySection title="Your contact details">
              ...
              <JourneySection title="Your event">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                  <PublicSelect
                    label="Event type"
                  ...
                  {isSingleCategory ? (
                    <div className="flex min-w-0 items-center gap-2 rounded-xl border journey-border journey-soft px-3">
                      <span className="journey-body journey-muted">
                        Category:
                      </span>
                    ...
                    <PublicSelect
                      ...
                      onChange={(e) =>
                        setField("enquiry_category_slug", e.target.value)
                      }
                    >
                  ...
                  <PublicSelect
                    ...
                    onChange={(e) =>
                      setField("contact_preference", e.target.value)
                    }
                  >
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <label className="min-w-0">
                    <span className="mb-1 block journey-body font-medium journey-text">
                      Date
                    </span>
                    <Input
                      type="date"
              ...
              <fieldset className="journey-form-section">
                <legend className="px-1 journey-body font-semibold journey-ink">
                  Budget
                </legend>
                <div className="space-y-3">
                  <label className="block min-w-0">
                    <span className="mb-1 block journey-body journey-text">
                      Amount
                    </span>
                    <div className="relative min-w-0">
                      <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 journey-body journey-muted">
                        £
                      </span>
                      <Input
                        type="number"
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      type="button"
                      aria-pressed={form.budget_unit === "per_person"}
                      variant={
                        form.budget_unit === "per_person"
                          ? "primary"
                          : "secondary"
                      }
                    ...
                    <Button
                      type="button"
                      aria-pressed={form.budget_unit === "in_total"}
                      variant={
                        form.budget_unit === "in_total"
                          ? "primary"
                          : "secondary"
              ...
              <JourneySection title="Venue and requirements">
                <div className="rounded-xl border journey-border journey-soft p-3">
                  <label className="inline-flex items-center gap-2 journey-body journey-text">
                    <input
                      type="checkbox"
                      checked={form.venue_known}
                      onChange={(e) =>
                        setField("venue_known", e.target.checked)
                      }
                    />
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
                    <div className="relative">
                      ...
                      />
                      {showVenueResults &&
                      (venueResults.length > 0 || venuesLoading) ? (
                        <div className="absolute z-20 mt-1 max-h-60 w-full overflow-auto rounded-xl border journey-border bg-white shadow-lg">
                          {venuesLoading ? (
                            <div className="px-3 py-2 journey-body journey-muted">
                              Searching venues...
                            </div>
                          ) : (
                            venueResults.map((v) => (
                              <button
                                ...
                                className="block w-full px-3 py-2 text-left journey-body hover:journey-soft"
                              >
                                <div className="font-medium journey-ink">
                                  {v.name}
                                </div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setField("venue_postcode", e.target.value)
                      }
                    />
                  </div>
                </div>
                ...
                {showPowerToggle ? (
                  <div className="rounded-xl border journey-border journey-soft p-3">
                    <p className="journey-body font-medium journey-ink">
                      Is power available on-site?
                    </p>
                    <div className="mt-2 flex flex-wrap gap-2">
                      <Button
                        type="button"
                        aria-pressed={form.power_available === true}
                        variant={
                          form.power_available === true
                            ? "primary"
                            : "secondary"
                        }
                        onClick={() => setField("power_available", true)}
                      >
                        Yes
                      </Button>
                      <Button
                        type="button"
                        aria-pressed={form.power_available === false}
                        variant={
                          form.power_available === false
                            ? "primary"
                            : "secondary"
                        }
                        onClick={() => setField("power_available", false)}
                      >
                        No
                      </Button>
                      <Button
                        type="button"
                        aria-pressed={form.power_available === null}
                        variant={
                          form.power_available === null
                            ? "primary"
                            : "secondary"
                        }
                        onClick={() => setField("power_available", null)}
                      >
                        Not sure
                      </Button>
                    </div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setField("dietary_requirements", e.target.value)
                  }
                />
              </JourneySection>
              <JourneySection title="Tell the supplier more">
                <div>
                  <p className="mb-1 journey-body journey-muted">{prompt}</p>
                  <textarea
                    aria-label="Event details"
                    aria-describedby="request-message-help"
                    className="min-h-[140px] w-full rounded-xl border journey-border bg-white px-3 py-2 journey-body shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/25"
                  ...
                  <p
                    ...
                    className={`mt-1 journey-small ${messageLength < 80 ? "journey-warning" : "journey-muted"}`}
                  >
              ...
              {suggestions.length > 0 || serverHints.length > 0 ? (
                <div className="rounded-xl border journey-warning-border journey-warning-surface px-3 py-2 journey-body journey-warning">
                  Suggestions:{" "}
                  {Array.from(new Set([...suggestions, ...serverHints])).join(
                    " | ",
                  )}
                </div>
              ) : null}
              ...
              {error ? (
                <p role="alert" className="journey-body journey-danger">
                  {error}
                </p>
              ) : null}

import { Section } from "../../components/workspace/AdminPrimitives";
...
function renderVenue(venue, locationLabel) {
  const parts = [
    venue?.name,
    venue?.address,
    venue?.locationLabel || locationLabel,
  ].filter(Boolean);
      ...
      <div className={className}>
        <div className="grid grid-cols-1 gap-2 text-sm sm:grid-cols-2 lg:grid-cols-3">
          ...
          <p>
            ...
            {enquiry.categoryLabel || "-"}
          </p>
        </div>
        <p className="mt-2 text-sm">
          <span className="text-slate-500">Venue:</span>{" "}
          <span className="text-slate-800">
            {renderVenue(enquiry.venue, enquiry.locationLabel)}
          </span>
        </p>
        {enquiry.message ? (
          <div className="mt-4 whitespace-pre-wrap">
            {compact
              ? enquiry.shortMessagePreview || enquiry.message
              : enquiry.message}
          </div>
        ) : null}
      </div>
